...
module.exports = (req, services) => ({
  dataloaders: {
    messages: new Dataloaders((messageIds) => services.messageProvider
    ...
    getMessageByUser: new Dataloaders(async (userIds) => {
      const messages = await services.messageProvider.find({ userId: { $in: userIds } });
      return userIds.map((id) => messages.filter((message) => message.userId.equals(id)));
    }),
    users: new Dataloaders((userIds) => services.userProvider
      .find({ _id: { $in: userIds } })
